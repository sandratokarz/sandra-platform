...
const s = {
  ...
  back: { fontSize: '10px', color: 'rgba(255,255,255,0.3)', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' as const, padding: 0, fontFamily: 'Inter, sans-serif' },
  num: { fontSize: '8px', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.2)', marginTop: '8px' },
  mlTitle: { fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 800, color: '#fff', lineHeight: 1.35, marginBottom: '14px' },
  ...
  stepTxt: { fontSize: '10px', color: '#fff' },
  right: { overflowY: 'auto' as const, padding: '1.5rem 2rem', background: '#F5F0E8', flex: 1 },
  eye: { fontSize: '9px', color: '#9B96C0', marginBottom: '6px' },
  title: { fontFamily: 'Syne, sans-serif', fontSize: '17px', fontWeight: 800, color: '#1A1A2E', marginBottom: '8px', lineHeight: 1.2 },
  body: { fontSize: '12px', color: '#5B5680', lineHeight: 1.7, marginBottom: '14px' },
  ...
  btn: { background: '#7B5CF5', color: '#fff', border: 'none', borderRadius: '7px', padding: '8px 16px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Syne, sans-serif' },
  btnGhost: { background: 'transparent', color: '#7B5CF5', border: '1px solid #7B5CF5', borderRadius: '7px', padding: '8px 16px', fontSize: '11px', cursor: 'pointer', fontFamily: 'Inter, sans-serif' },
  chip: { display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '8px', color: '#5B5680', background: '#EDE8DF', borderRadius: '5px', padding: '2px 7px' },
  ...
  quote: { background: '#EDE9FE', borderLeft: '3px solid #7B5CF5', borderRadius: '0 8px 8px 0', padding: '10px 13px', fontSize: '12px', color: '#4C1D95', fontStyle: 'italic' as const, lineHeight: 1.6, marginBottom: '14px' },
  upNext: { marginTop: 'auto', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.06)' },
  upNextLbl: { fontSize: '7px', letterSpacing: '0.1em', color: 'rgba(255,255,255,0.2)', marginBottom: '5px' },
  upNextItem: { display: 'flex', alignItems: 'center', gap: '6px', fontSize: '10px', color: 'rgba(255,255,255,0.5)' },
}
      ...
      <div style={s.right}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '14px' }}>
            {[
              ...
              <div key={c.title} style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: '11px', padding: '1rem' }}>
                ...
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 700, color: c.titleColor, marginBottom: '6px' }}>{c.title}</div>
                {c.items.map(it => <div key={it} style={{ fontSize: '10px', color: c.titleColor, lineHeight: 1.45, display: 'flex', gap: '5px', marginBottom: '2px' }}><span>·</span>{it}</div>)}
                <div style={{ fontSize: '9px', fontStyle: 'italic', marginTop: '8px', padding: '5px 8px', borderRadius: '5px', background: c.qBg, color: c.titleColor, lineHeight: 1.5 }}>{c.q}</div>
              </div>
            ))}
          </div>
        ...
        {step === 1 && <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '12px', flexWrap: 'wrap' as const }}>
            {sfData.map((sf, i) => (
              <>
                <div key={i} onClick={() => setSfStep(i)} style={{ cursor: 'pointer', background: sfStep === i ? '#7B5CF5' : '#fff', border: `1px solid ${sfStep === i ? '#7B5CF5' : 'rgba(0,0,0,0.08)'}`, borderRadius: '8px', padding: '8px 11px', transition: 'all .15s' }}>
                  <div style={{ fontSize: '10px', fontWeight: 600, color: sfStep === i ? '#fff' : '#1A1A2E' }}>{['Priorities', 'Use Cases', 'Learning', 'Adoption', 'Impact'][i]}</div>
                  <div style={{ fontSize: '8px', color: sfStep === i ? 'rgba(255,255,255,0.6)' : '#9B96C0' }}>{['Define focus', 'Real work', 'Performance', 'Behaviour', 'Measure'][i]}</div>
                </div>
                {i < 4 && <span key={'arr' + i} style={{ fontSize: '14px', color: '#9B96C0' }}>→</span>}
              </>
            ))}
          </div>
          <div style={{ background: '#EDE9FE', border: '1px solid rgba(123,92,245,0.2)', borderRadius: '10px', padding: '1rem', marginBottom: '12px' }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '13px', fontWeight: 700, color: '#4C1D95', marginBottom: '6px' }}>{sfData[sfStep].title}</div>
            <div style={{ fontSize: '11px', color: '#1A1A2E', lineHeight: 1.65, marginBottom: '8px' }}>{sfData[sfStep].body}</div>
            <div style={{ fontSize: '11px', color: '#7B5CF5', fontStyle: 'italic', borderLeft: '2px solid #7B5CF5', paddingLeft: '8px' }}>{sfData[sfStep].q}</div>
          </div>
        ...
        {step === 2 && <>
          ...
          <div style={s.card}>
            <div style={{ fontSize: '8px', color: '#7B5CF5', fontWeight: 500, letterSpacing: '0.06em', marginBottom: '8px' }}>Prosus · Finance Transformation · 2022</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
              <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>The situation</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Finance had near-zero AI literacy. Learning was a one-off event. Leadership wanted digital transformation but had no L&D strategy.</div></div>
              <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>What Sandra did</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Created a strategy aligned to Finance capability goals — moving from one-off events to role-specific, tool-connected, manager-supported learning embedded in daily work.</div></div>
            </div>
            <div style={s.outcome}><div style={{ fontSize: '8px', fontWeight: 600, color: '#065F46', marginBottom: '2px' }}>OUTCOME</div><div style={{ fontSize: '10px', color: '#1A1A2E', lineHeight: 1.5 }}>Finance moved from resistance to active AI adoption in one quarter. Became the internal case study for L&D-led digital transformation. Replicated across Legal and HR.</div></div>
    ...
    <div style={s.shell}>
      ...
      <div style={s.right}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '12px' }}>
            {roles.map((role, i) => (
              <div key={i} onClick={() => setActiveRole(i)} style={{ background: '#fff', border: `1.5px solid ${activeRole === i ? '#7B5CF5' : 'rgba(0,0,0,0.07)'}`, borderRadius: '11px', overflow: 'hidden', cursor: 'pointer' }}>
                <div style={{ height: '70px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', background: role.bg }}>{role.emoji}</div>
                <div style={{ padding: '10px 11px' }}>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '11px', fontWeight: 700, color: '#1A1A2E', marginBottom: '2px', lineHeight: 1.3 }}>{role.title}</div>
                  <div style={{ fontSize: '9px', color: '#5B5680' }}>{role.sub}</div>
                  <div style={{ fontSize: '8px', padding: '2px 6px', borderRadius: '5px', marginTop: '5px', display: 'inline-block', background: activeRole === i ? '#EDE9FE' : '#EDE8DF', color: activeRole === i ? '#4C1D95' : '#9B96C0' }}>{activeRole === i ? 'Active' : 'Click to view'}</div>
                </div>
              </div>
            ))}
          </div>
          <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '11px', padding: '1.1rem', marginBottom: '12px' }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '13px', fontWeight: 700, color: '#1A1A2E', marginBottom: '4px' }}>{r.curriculum.title}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', marginBottom: '10px' }}>
              {r.curriculum.outcomes.map(o => <div key={o} style={{ fontSize: '10px', color: '#5B5680', display: 'flex', gap: '5px' }}><span style={{ color: '#1D9E75', fontSize: '9px', marginTop: '1px' }}>✓</span>{o}</div>)}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {r.curriculum.modules.map((m, mi) => (
                <div key={mi} style={{ background: '#F5F0E8', borderRadius: '7px', padding: '8px 10px' }}>
                  <div style={{ fontSize: '8px', color: colors[activeRole], marginBottom: '3px' }}>{m.n}</div>
                  {m.items.map(it => <div key={it} style={{ fontSize: '10px', color: '#5B5680', display: 'flex', gap: '5px', marginBottom: '2px' }}><span style={{ fontSize: '9px', flexShrink: 0 }}>{it.split(' ')[0]}</span><span>{it.split(' ').slice(1).join(' ')}</span></div>)}
                </div>
              ))}
            </div>
          </div>
        ...
        {step === 1 && <>
          ...
          <div style={s.card}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
              <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>Prosus — Finance Growth Curriculum</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Built role-based curriculum for Finance aligned to capability goals and career stages. Learning became purposeful, not generic.</div></div>
              <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>Booking.com — Capability Maps</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Implemented capability learning maps for 18k associates as part of a full career framework. People could see their growth path clearly.</div></div>
            </div>
      ...
      <div style={s.right}>
        {step === 0 && <>
          ...
          <div style={s.card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              <div style={{ width: '24px', height: '24px', borderRadius: '6px', background: '#EDE9FE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px' }}>✦</div>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '13px', fontWeight: 700, color: '#1A1A2E' }}>AI Programme Builder</div>
            </div>
            <div style={{ fontSize: '11px', color: '#5B5680', marginBottom: '10px', lineHeight: 1.6 }}>Input your challenge and audience — get a full learning programme with modules, objectives, and activities.</div>
            <textarea rows={2} value={challenge} onChange={e => setChallenge(e.target.value)} placeholder="e.g. 'KF consultants need to use AI tools in client projects but are resistant to starting'" style={{ width: '100%', background: '#F5F0E8', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '7px', padding: '8px 11px', fontSize: '11px', color: '#1A1A2E', fontFamily: 'Inter, sans-serif', outline: 'none', resize: 'none', marginBottom: '8px', boxSizing: 'border-box' }} />
            <div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap' as const }}>
              <select value={audience} onChange={e => setAudience(e.target.value)} style={{ background: '#F5F0E8', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '6px', padding: '5px 9px', fontSize: '10px', color: '#5B5680', fontFamily: 'Inter, sans-serif', outline: 'none' }}>
              ...
              <select value={format} onChange={e => setFormat(e.target.value)} style={{ background: '#F5F0E8', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '6px', padding: '5px 9px', fontSize: '10px', color: '#5B5680', fontFamily: 'Inter, sans-serif', outline: 'none' }}>
                <option value="blended">Blended</option><option value="online">Self-paced</option><option value="workshop">Workshop</option>
              </select>
              <button onClick={generate} disabled={loading} style={{ ...s.btn, fontSize: '10px', padding: '5px 14px', opacity: loading ? 0.7 : 1 }}>{loading ? '⟳ Generating…' : '✦ Generate'}</button>
            </div>
            {result && !result.error && (
              <div style={{ marginTop: '12px' }}>
                <div style={{ fontSize: '11px', fontWeight: 500, color: '#1A1A2E', marginBottom: '5px' }}>{result.title}</div>
                <div style={{ fontSize: '10px', color: '#5B5680', marginBottom: '8px', lineHeight: 1.55 }}>{result.overview}</div>
                {(result.modules || []).map((m: any) => (
                  <div key={m.number} style={{ background: '#F5F0E8', borderRadius: '7px', padding: '8px 10px', marginBottom: '5px', borderLeft: '2.5px solid #7B5CF5' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}><div style={{ fontSize: '11px', fontWeight: 500, color: '#1A1A2E' }}>Module {m.number}: {m.title}</div><div style={{ fontSize: '8px', color: '#9B96C0' }}>{m.duration} · {m.format}</div></div>
                    <div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5, marginBottom: '4px' }}>{m.description}</div>
                    {(m.objectives || []).map((o: string) => <div key={o} style={{ fontSize: '9px', color: '#5B5680', paddingLeft: '9px', position: 'relative', lineHeight: 1.4, marginBottom: '1px' }}><span style={{ position: 'absolute', left: 0, color: '#1D9E75', fontSize: '8px' }}>✓</span>{o}</div>)}
                    {m.activity && <div style={{ marginTop: '5px', padding: '5px 8px', background: '#EDE9FE', borderRadius: '5px', fontSize: '9px', color: '#4C1D95' }}><strong>Activity: </strong>{m.activity}</div>}
                  </div>
                ))}
                {result.outcomes && <div style={{ background: '#EDFAF5', border: '1px solid rgba(29,158,117,0.2)', borderRadius: '7px', padding: '8px 10px', fontSize: '9px', color: '#065F46', lineHeight: 1.5 }}><strong style={{ display: 'block', marginBottom: '2px' }}>OUTCOMES</strong>{result.outcomes}</div>}
              </div>
            )}
          </div>
          <div style={s.nav}><button style={s.btn} onClick={() => setStep(1)}>Next →</button></div>
        ...
        {step === 1 && <>
          ...
          <div style={{ ...s.card, marginBottom: '8px' }}>
            <div style={{ fontSize: '8px', color: '#A78BFA', fontWeight: 500, letterSpacing: '0.06em', marginBottom: '5px' }}>Sample 1 · Booking.com · 2019</div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 700, color: '#1A1A2E', marginBottom: '6px' }}>Global Hiring Manager Toolkit</div>
            <div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5, marginBottom: '8px' }}>eLearning + competency toolkit for 18k associates. Became the global standard from day one of launch.</div>
            <span style={s.chip}><span style={s.chipDot} />18k associates · global standard</span>
          </div>
          <div style={s.card}>
            <div style={{ fontSize: '8px', color: '#1D9E75', fontWeight: 500, letterSpacing: '0.06em', marginBottom: '5px' }}>Sample 2 · Prosus · 2021</div>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 700, color: '#1A1A2E', marginBottom: '6px' }}>Inclusion Workshop Programme</div>
            <div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5, marginBottom: '8px' }}>Full workshop design with facilitation guide, action planning toolkit, leadership reporting. 10+ point improvement in engagement scores. Rolled out globally.</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
          {[
            ...
            <div key={c.title} style={s.card}>
              <div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>{c.title}</div>
              <div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5, marginBottom: '6px' }}>{c.desc}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '14px' }}>
          {['Certified vILT instructor — online facilitation at scale', 'Master Train the Trainer — developing facilitators across organisations', '5 languages · delivered in English, German, Polish and via interpreters'].map(t => (
            <div key={t} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: '#fff', borderRadius: '8px', padding: '9px 12px', border: '1px solid rgba(0,0,0,0.07)' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: '#378ADD', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', color: '#fff', flexShrink: 0 }}>✓</div>
              <div style={{ fontSize: '11px', color: '#1A1A2E' }}>{t}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '12px' }}>
          {[['Recruit', 'Select by influence, not just enthusiasm'], ['Equip', 'Playbooks, talking points, local use cases'], ['Activate', 'Drive adoption locally, with L&D as strategic support']].map(([t, d]) => (
            <div key={t} style={{ ...s.card, textAlign: 'center' as const, marginBottom: 0 }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '13px', fontWeight: 700, color: '#7B5CF5', marginBottom: '4px' }}>{t}</div>
              <div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>{d}</div>
            </div>
          ))}
        </div>
        <div style={s.card}>
          <div style={{ fontSize: '8px', color: '#7B5CF5', fontWeight: 500, letterSpacing: '0.06em', marginBottom: '8px' }}>Case Study · Mibelle Group · 2024–25</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>The situation</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Post-acquisition. People were uncertain. Top-down comms wouldn't create the adoption speed needed across global teams.</div></div>
            <div><div style={{ fontSize: '10px', fontWeight: 600, color: '#1A1A2E', marginBottom: '3px' }}>What Sandra did</div><div style={{ fontSize: '10px', color: '#5B5680', lineHeight: 1.5 }}>Recruited, trained, and equipped AI Champions across all regions with full plans, role-based toolkits, and autonomy to add their own ideas.</div></div>
          </div>
          <div style={s.outcome}><div style={{ fontSize: '8px', fontWeight: 600, color: '#065F46', marginBottom: '2px' }}>OUTCOME</div><div style={{ fontSize: '10px', color: '#1A1A2E', lineHeight: 1.5 }}>Champions became the connective tissue of the transition. Local adoption accelerated. The model became the template for future change programmes at the organisation.</div></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '7px', marginBottom: '14px' }}>
          {[['4×', 'Faster adoption vs top-down comms'], ['↓', 'Reduced L&D support burden'], ['↑', 'Sustained engagement beyond launch']].map(([num, txt]) => (
            <div key={txt} style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '9px', padding: '10px', textAlign: 'center' as const }}>
              <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '22px', fontWeight: 800, color: '#7B5CF5' }}>{num}</div>
              <div style={{ fontSize: '9px', color: '#5B5680', marginTop: '2px', lineHeight: 1.4 }}>{txt}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '9px', marginBottom: '14px' }}>
            {[
              ...
              <div key={c.label} style={{ ...s.card, borderTop: `3px solid ${c.color}`, marginBottom: 0 }}>
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 700, color: c.color, marginBottom: '5px' }}>{c.label}</div>
                <div style={{ fontSize: '10px', color: '#1A1A2E', fontWeight: 500, marginBottom: '4px' }}>{c.q}</div>
                <div style={{ fontSize: '9px', color: '#5B5680', lineHeight: 1.6, marginBottom: '4px' }}>{c.items}</div>
                <div style={{ fontSize: '9px', color: c.color, fontStyle: 'italic', marginTop: '6px' }}>{c.note}</div>
              </div>
            ))}
          </div>
          <div style={s.card}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '12px', fontWeight: 700, color: '#1A1A2E', marginBottom: '10px' }}>Continuous Iteration Engine</div>
            <div style={{ display: 'flex', alignItems: 'stretch' }}>
              <div style={{ flex: 1, background: '#EDE9FE', borderRadius: '8px 0 0 8px', padding: '10px' }}><div style={{ fontSize: '9px', fontWeight: 600, color: '#4C1D95', marginBottom: '4px' }}>Step 1: Identify signals</div><div style={{ fontSize: '9px', color: '#7B5CF5', lineHeight: 1.5 }}>Low adoption → friction · High usage low impact → poor use case · High engagement low application → wrong format</div></div>
              <div style={{ width: '1px', background: 'rgba(123,92,245,0.15)' }} />
              <div style={{ flex: 1, background: '#FDF2F2', padding: '10px' }}><div style={{ fontSize: '9px', fontWeight: 600, color: '#9B1C1C', marginBottom: '4px' }}>Step 2: Take action</div><div style={{ fontSize: '9px', color: '#9B1C1C', lineHeight: 1.5 }}>Refine use cases · Redesign learning · Strengthen champion involvement</div></div>
              <div style={{ width: '1px', background: 'rgba(244,168,168,0.3)' }} />
              <div style={{ flex: 1, background: '#EDFAF5', borderRadius: '0 8px 8px 0', padding: '10px' }}><div style={{ fontSize: '9px', fontWeight: 600, color: '#065F46', marginBottom: '4px' }}>Step 3: Scale what works</div><div style={{ fontSize: '9px', color: '#065F46', lineHeight: 1.5 }}>Double down on high-impact use cases · Expand into advanced learning · Share success stories</div></div>
            </div>
          </div>
        ...
        {step === 1 && <>
          ...
          <div style={{ display: 'flex', gap: '7px', marginBottom: '12px', flexWrap: 'wrap' as const }}>
            {['All departments', 'Consulting', 'Leadership', 'Internal Functions'].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ fontSize: '10px', padding: '4px 10px', borderRadius: '14px', border: `1px solid ${filter === f ? '#7B5CF5' : 'rgba(0,0,0,0.08)'}`, background: filter === f ? '#7B5CF5' : '#fff', color: filter === f ? '#fff' : '#5B5680', cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>{f}</button>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px', marginBottom: '12px' }}>
            {[['71%', 'Active AI users', '#7B5CF5'], ['84%', 'Module completion', '#A78BFA'], ['+22pt', 'Confidence score', '#1D9E75'], ['3.2h', 'Avg time saved/wk', '#BA7517']].map(([num, lbl, color]) => (
              <div key={lbl} style={{ ...s.card, position: 'relative', overflow: 'hidden', marginBottom: 0 }}>
                ...
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '22px', fontWeight: 800, color: '#1A1A2E', lineHeight: 1 }}>{num}</div>
                <div style={{ fontSize: '9px', color: '#5B5680', marginTop: '2px' }}>{lbl}</div>
              </div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
            <div style={s.card}>
              <div style={{ fontSize: '12px', fontWeight: 500, color: '#1A1A2E', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#7B5CF5' }} />AI adoption by role</div>
              {[['Consultants', 86, '#7B5CF5'], ['Leaders', 58, '#A78BFA'], ['Internal teams', 71, '#1D9E75']].map(([lbl, pct, color]) => (
                <div key={lbl as string} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '7px' }}>
                  <div style={{ fontSize: '10px', color: '#5B5680', minWidth: '100px' }}>{lbl}</div>
                  ...
                  <div style={{ fontSize: '10px', fontWeight: 500, color: '#1A1A2E', minWidth: '30px', textAlign: 'right' }}>{pct}%</div>
                </div>
              ))}
              <div style={{ background: '#EDE9FE', border: '1px solid rgba(123,92,245,0.18)', borderRadius: '7px', padding: '8px 10px', marginTop: '8px' }}><div style={{ fontSize: '8px', fontWeight: 500, color: '#4C1D95', marginBottom: '2px', letterSpacing: '0.06em' }}>INSIGHT</div><div style={{ fontSize: '10px', color: '#7B5CF5' }}>Leaders require stronger enablement for activation — priority for Q2</div></div>
            </div>
            <div style={s.card}>
              <div style={{ fontSize: '12px', fontWeight: 500, color: '#1A1A2E', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#1D9E75' }} />Learning → Application funnel</div>
              {[['Completed', '84%', '90%', '#7B5CF5'], ['Practiced', '71%', '75%', '#A78BFA'], ['Applied', '52%', '55%', '#1D9E75']].map(([lbl, val, w, color]) => (
                <div key={lbl as string} style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px' }}>
                  <div style={{ fontSize: '10px', color: '#5B5680', minWidth: '80px' }}>{lbl}</div>
                  <div style={{ height: '20px', width: w as string, background: color as string, borderRadius: '4px', display: 'flex', alignItems: 'center', paddingLeft: '8px' }}><span style={{ fontSize: '9px', fontWeight: 600, color: '#fff' }}>{val}</span></div>
                </div>
              ))}
            </div>
          </div>
          <div style={s.card}>
            <div style={{ fontSize: '12px', fontWeight: 500, color: '#1A1A2E', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#BA7517' }} />Use case performance — Proposal Creation</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', textAlign: 'center' as const }}>
              {[['78%', 'Consultant adoption rate', '#7B5CF5'], ['-45min', 'Avg time saved per proposal', '#1D9E75'], ['4.2/5', 'Output quality rating', '#A78BFA']].map(([num, lbl, color]) => (
                <div key={lbl as string}><div style={{ fontFamily: 'Syne, sans-serif', fontSize: '20px', fontWeight: 800, color: color as string }}>{num}</div><div style={{ fontSize: '9px', color: '#5B5680', marginTop: '2px' }}>{lbl}</div></div>
              ))}
            </div>
          </div>
          <div style={s.nav}><button style={s.btnGhost} onClick={() => setStep(0)}>← Back</button><button style={s.btn} onClick={() => setStep(2)}>Next →</button></div>
