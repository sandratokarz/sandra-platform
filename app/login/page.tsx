...
export default function LoginPage() {
  ...
  function handleLogin() {
    if (password === 'kornferry2026') {
      router.push('/platform')
    } else if (password === 'systemiq2026') {
      router.push('/platform/systemiq')
    } else {
    ...
    <main style={{ backgroundColor: '#1A1A2E', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '380px' }}>
        ...
        <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.35)', lineHeight: 1.6, marginBottom: '20px' }}>
          Enter your access code to explore the interactive platform.
        </p>
        ...
        {error && <p style={{ fontSize: '11px', color: '#F4A8A8', marginBottom: '8px' }}>{error}</p>}

        <button
          onClick={handleLogin}
          style={{ width: '100%', background: 'linear-gradient(135deg, #7B5CF5, #F4A8A8)', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'Inter, sans-serif', marginTop: '4px' }}>
          Enter Platform →
        </button>
        ...
        <p onClick={() => router.push('/')} style={{ textAlign: 'center', fontSize: '10px', color: 'rgba(255,255,255,0.25)', marginTop: '10px', cursor: 'pointer' }}>
          ← Back to home
        </p>
      </div>
    </main>
